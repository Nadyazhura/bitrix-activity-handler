...
Обработчик вебхуков для Bitrix24, который автоматически маршрутизирует входящие email-активности к соответствующим сделкам или лидам на основе совпадения тем.
...
2. Создайте файл `.env` в корневой директории со следующими переменными:
   ...
   ```
3. Обновите `config/config.php` с вашим URL входящего вебхука Bitrix24 и настройками логирования
4. Убедитесь, что директория `logs/` доступна для записи веб-сервером (при использовании файлового логирования)
...
3. Извлекает тему
4. Ищет по всем лидам и сделкам 
5. Есди совпадение найдено - прикрепляет активность к найденому лиду или сделке
6. Если нет совпадений, создает новый лид и клонирует активность
...
Логи записываются в `syslog` по умолчанию. Уровни логирования:
